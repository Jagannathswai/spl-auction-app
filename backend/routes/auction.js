...
router.put('/:roomId/sold', protect, adminOnly, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: player.name + ' sold to ' + team.name + ' for Rs.' + soldPrice + 'L', player, team });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
...
// PUT /api/auction/:roomId/re-auction-unsold - reset unsold players to available
router.put('/:roomId/re-auction-unsold', protect, adminOnly, async (req, res) => {
  try {
    const result = await Player.updateMany(
      { room: req.params.roomId, status: 'unsold' },
      { status: 'available', soldPrice: 0, soldTo: null }
    );
    req.io.to(req.params.roomId).emit('auction:reset-unsold', { count: result.modifiedCount });
    res.json({ success: true, message: result.modifiedCount + ' players reset to available' });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
